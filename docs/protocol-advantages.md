...
Context Graph Protocol exists to make every one of those questions answerable, not by convention,
...
| **Conformance verification** | "Context Graph Protocol conformant" is a machine-checked claim, not a self-attestation; the conformance suite is adversarial | `contextgraph-conformance` — 5 checks that deliberately trip each failure mode |
...
`by: "contextgraph-graph"`. A frame from an episodic memory store might carry an
...
In Context Graph Protocol, the budget is part of the *query contract*, and the provider is
...
The `DataFlow` struct is the security-critical field in Context Graph Protocol:
...
critical for enterprise security, compliance, and trust. Context Graph Protocol's consent model
...
"Context Graph Protocol conformant" is not a self-attestation. It is a machine-checked claim,
defined as **green on `contextgraph-conformance`'s suite for your declared capability
...
Context Graph Protocol separates **crate version** (ordinary Cargo semver) from **protocol
version** (the wire-format identity negotiated at handshake). The current
protocol version is `contextgraph/1.0-draft`.

Two protocol versions interoperate when they share a **major family** — the
substring up to the first `.`. So `contextgraph/1.0-draft` and `contextgraph/1.0` both belong to
...
becomes `contextgraph/1.0`, every already-deployed `1.0-draft` provider continues to
handshake successfully — they share the `1` family. What breaks interop is
reserved for a genuinely breaking redesign (`contextgraph/2.0`), which would require a
...
freezing the spec to avoid disruption. Context Graph Protocol's major-family model allows
...
This is the property that makes Context Graph Protocol suitable for **long-running,
...
- **Remove consent enforcement.** Any provider can exfiltrate workspace content
  to a remote service. The "no phone-home" guarantee — central to Context Graph Protocol's
  trust model — becomes unenforceable at the protocol level. → *Data leakage.*

- **Remove conformance verification.** "Context Graph Protocol conformant" becomes a
...
does not address. This is why Context Graph Protocol is specified as an integrated protocol, not
...
resources to LLM-based applications. Context Graph Protocol and MCP are complementary, not
competing:

- **MCP** connects *tools* — functions the agent can call (run a query, fetch
  a resource, execute a command). It is an action protocol.
- **Context Graph Protocol** connects *context* — typed, budgeted, provenance-carrying frames
...
is designed for tool invocation; Context Graph Protocol is designed for evidence retrieval. An
agent that needs both composes them: Context Graph Protocol providers feed context into the
prompt, MCP tools execute actions.

The architectural distinction is that Context Graph Protocol frames are **transported as untrusted
...
well-designed hosts, but Context Graph Protocol makes the untrusted-data contract part of the
...
a vector store and pastes them into the prompt. Compared to Context Graph Protocol:

- **No budget contract.** The retriever returns top-K; the prompt assembler
  hopes they fit. Context Graph Protocol's `max_tokens` is part of the query and enforced.
- **No provenance.** Chunks carry a source document, at best. Context Graph Protocol frames
  carry URI, range, digest, method, and agent.
- **No consent model.** A cloud embedding API is called without gating. Context Graph Protocol
  gates egress behind recorded, named consent.
- **No conformance.** There is no way to verify a RAG pipeline respects any
  contract. Context Graph Protocol defines conformance as machine-checked.
- **No temporal validity.** Chunks are current-or-not, with no validity
  window. Context Graph Protocol frames carry bi-temporal metadata.
...
provenance, or consent. Context Graph Protocol inverts this: retrieval is an open protocol, the
...
The design of Context Graph Protocol is grounded in research on retrieval-augmented generation,
...
- **Lost in the Middle** (Liu et al., TACL 2024,
  ...
  contexts. Context Graph Protocol's budget-honesty contract and frame-level relevance scoring
...
- **Context Rot** (Hong et al., Chroma, 2025,
  ...
  additional tokens are relevant. This validates Context Graph Protocol's position that *more
...
- **Graph RAG** (Edge et al., Microsoft Research, 2024,
  ...
  flat vector search for global questions about a corpus. Context Graph Protocol's `Relation`
...
- **Repo map with tree-sitter** (Gauthier, Aider, 2023,
  ...
  structural awareness that grep cannot. Context Graph Protocol's `FrameKind::Symbol` and
...
- **AI Agents That Matter** (Kapoor et al., Princeton, TMLR 2025,
  ...
  more accurate but 10x more expensive is not necessarily better. Context Graph Protocol's
...
- **MemGPT** (Packer et al., UC Berkeley, 2023,
  ...
  context). Context Graph Protocol's frame types (`Memory`, `Episode`, `Fact`) and temporal
...
The Context Graph Protocol is not a faster retrieval pipeline or a richer
...
The implementation is open (`contextgraph-types`, `contextgraph-host`, `contextgraph-conformance`),
